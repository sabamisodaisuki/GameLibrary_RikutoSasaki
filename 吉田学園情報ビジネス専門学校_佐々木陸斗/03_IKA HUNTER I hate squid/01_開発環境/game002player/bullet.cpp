...
void lnitBullet(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data/TEXTURE/ami.png",					//akira000.png,koda.jpg,su.jpg,runningman100.png,bullet000.png,人参.png,金人参.png,ぺこら.jpg
		&g_pTextureBullet);
...
void UpdateBullet(void)
{
	...
	for (nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		...
		{//弾が使用される
			...
			//弾の位置の更新
			pVtx[0].pos = D3DXVECTOR3(g_aBullet[nCntBullet].pos.x - BULLET_SIZ, g_aBullet[nCntBullet].pos.y - BULLET_SIZ, 0.0f);
			pVtx[1].pos = D3DXVECTOR3(g_aBullet[nCntBullet].pos.x + BULLET_SIZ, g_aBullet[nCntBullet].pos.y - BULLET_SIZ, 0.0f);
			pVtx[2].pos = D3DXVECTOR3(g_aBullet[nCntBullet].pos.x - BULLET_SIZ, g_aBullet[nCntBullet].pos.y + BULLET_SIZ, 0.0f);
			pVtx[3].pos = D3DXVECTOR3(g_aBullet[nCntBullet].pos.x + BULLET_SIZ, g_aBullet[nCntBullet].pos.y + BULLET_SIZ, 0.0f);
			...
			for (nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++,pEnemy++)
			{
				if (pEnemy->bUse == true)
				{//敵がいるとき
					if (pEnemy->pos.x + ENEMY_SIZE / 2 > g_aBullet[nCntBullet].pos.x &&
						pEnemy->pos.x - ENEMY_SIZE / 2 < g_aBullet[nCntBullet].pos.x &&
              			pEnemy->pos.y + ENEMY_SIZE / 2 > g_aBullet[nCntBullet].pos.y &&
						pEnemy->pos.y - ENEMY_SIZE / 2 < g_aBullet[nCntBullet].pos.y)
					{//敵と弾が当たった
