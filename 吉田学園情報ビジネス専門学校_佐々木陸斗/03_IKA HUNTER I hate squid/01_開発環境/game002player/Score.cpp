...
void lnitScore(void)
{
	...
	g_posScore = D3DXVECTOR3(880.0f,13.0f,0.0f);//位置を初期化する
	g_nScore = 0;								//値を初期化する
...
void SetScore(int nScore, D3DXVECTOR3 pos, float fWidth, float fHeight)
{
	...
	aPosTexU[0] = g_nScore % 1000000000 / 100000000;
	aPosTexU[1] = g_nScore % 100000000 / 10000000;
	aPosTexU[2] = g_nScore % 10000000 / 1000000;
	aPosTexU[3] = g_nScore % 1000000 / 100000;
	aPosTexU[4] = g_nScore % 100000 / 10000;
	aPosTexU[5] = g_nScore % 10000 / 1000;
	aPosTexU[6] = g_nScore % 1000 / 100;
	aPosTexU[7] = g_nScore % 100 / 10;
	aPosTexU[8] = g_nScore % 10 / 1;
	...
	{
		//テクスチャ座標
		pVtx[0].tex = D3DXVECTOR2((float)aPosTexU[nCntScore], 0.0f);
		pVtx[1].tex = D3DXVECTOR2((float)aPosTexU[nCntScore] + (1.0f / TEXTURE_U), 0.0f);
		pVtx[2].tex = D3DXVECTOR2((float)aPosTexU[nCntScore], 1.0f);
		pVtx[3].tex = D3DXVECTOR2((float)aPosTexU[nCntScore] + (1.0f / TEXTURE_U), 1.0f);
