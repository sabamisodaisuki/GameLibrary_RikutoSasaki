...
void lnitEnemy(void)
{
	...
	D3DXLoadMeshFromX("data/MODEL/kuruma2.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
	...
	D3DXLoadMeshFromX("data/MODEL/kuruma1.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
	...
	D3DXLoadMeshFromX("data/MODEL/kuruma.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&g_EnemyInfo[2].pBuffMatEnemy,
		NULL,
		&g_EnemyInfo[2].nNumMatEnemy,
		&g_EnemyInfo[2].MeshEnemy);
...
void UpdateEnemy(void)
{
	...
	if (g_nEnemyCounter % 460 == 0)
	{
		SetEnemy(D3DXVECTOR3(2300.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -7.0f), 0); //車1
		SetEnemy(D3DXVECTOR3(2450.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -5.0f), 1); //車1
		SetEnemy(D3DXVECTOR3(2600.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -7.0f), 0);//壁奥
		SetEnemy(D3DXVECTOR3(2750.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -5.0f), 2);//壁奥
	}
	else if (g_nEnemyCounter % 240 == 0)
	{
		SetEnemy(D3DXVECTOR3(2300.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -7.0f), 0); //車1
		SetEnemy(D3DXVECTOR3(2450.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -5.0f), 1); //車1
		SetEnemy(D3DXVECTOR3(2600.0f, 230.0f, -170.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, -7.0f), 0);//壁奥
	...
	for (int nCntEnemy = 0; nCntEnemy<MAX_Enemy; nCntEnemy++)
	{
		if (g_Enemy[nCntEnemy].bUse == true)//使用している場合
		{
				...
				//ブロックにめり込んだ
			{
				pPos->x = g_Enemy[nCntEnemy].pos.x + g_Enemy[nCntEnemy].size.x + pSize->x;
				pMove->x = 0.0f;
				DeathPlayer(D3DXVECTOR3(2000.0f, 260.0f, -590.0f));
				AddScore(-1);
				DetCount++;
				...
				if (GetCount <1)
				{
					...
					SetTrophy(D3DXVECTOR3(SCREEN_WIDTH, 140.0f, 0.0f), D3DXVECTOR3(-5.0f, 0.0f, 0.0f), 1);
					GetCount++;
					AddGetScore(13);
				}
			}
			...
			if (DetCount == 10)
			{
				//サウンドの再生
				PlaySound(SOUND_LABEL_SE_fanfare);
				SetTrophy(D3DXVECTOR3(SCREEN_WIDTH, 140.0f, 0.0f), D3DXVECTOR3(-5.0f, 0.0f, 0.0f), 5);
				DetCount++;
				AddGetScore(13);
			}
		}
